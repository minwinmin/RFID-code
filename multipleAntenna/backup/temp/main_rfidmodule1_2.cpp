...
void multipleAntennaDetection(char TemporaryWineTags[tagsNum][idNum]){
    ...
    if (nano2.check() == true) //Check to see if any new data has come in from module
    {
      ...
      else if (responseType == RESPONSE_IS_TAGFOUND)
      {
        ...
        if(jj == 0){
          SERIAL1.println("uniqueEPC is : ");
          SERIAL1.println(uniqueEPC.c_str());
          memcpy(TemporaryWineTags[0], uniqueEPC.c_str(), idNum);
          jj += 1;
        }
        ...
        if (arrayNumber != 0){
          ...
          if (count == 0){   
            memcpy(TemporaryWineTags[k], uniqueEPC.c_str(), idNum);
            k += 1;
          }
        }
  ...
  //現在の状況をglobal変数に格納する
  for(int i = 0; i<arrayNumber; i++){
      memcpy(global_CurrentWineTags[i],TemporaryWineTags[i], idNum);
  }
