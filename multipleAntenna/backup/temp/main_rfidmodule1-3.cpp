...
char global_CurrentWineTags_2[tagsNum][idNum] = {0};
...
void loop(){
  ...
  if(first_detect == 0){
    ...
    // 下記もglobal変数を渡せば良さそう
    multipleAntennaDetection(global_DifferenceWineTags);
  ...
  if(buttonState == HIGH){
    ...
    // 検証用
    //memset(global_CurrentWineTags, '\0', sizeof(global_CurrentWineTags));
...
void multipleAntennaDetection(char TemporaryWineTags[tagsNum][idNum]){
  ...
  // for(int i = 0; i < arrayNumber; i++){
  //   strcpy(TemporaryWineTags[i],"\0");
  // }

  //memset(TemporaryWineTags, sizeof(tagsNum), '\0');
