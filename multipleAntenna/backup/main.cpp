...
char originalID[][tagsNum][idNum] = {{"a0a1", "94ab", "a081"}, {"947b", "a0ca", "a0c9"}, {"a0d2", "955c", "a0aa"}, 
                                   {"94fb", "a0d1", "a0b2"}, {"a13" , "94d4", "a12" }, {"a0da", "9524", "a0fc"},
                                   {"a0e2", "a0fb", "94a3"}, {"952c", "a132", "a133"}, {"a113", "a12c", "94ca"}, 
                                   {"a12b", "a1c" , "9554"}, {"a169", "943a", "a15c"}, {"9433", "a149", "a16a"},
                                   {"a151", "93e2", "a164"}, {"a1a2", "a1a1", "941" }, {"a19b", "a181", "949a"}, 
                                   {"a17a", "a19a", "94f3"}, {"a1d1", "a1d2", "9561"}, {"a1b2", "a1cb", "958b"},
                                   {"a1ca", "a1ab", "953c"}, {"a1fb", "a1fc", "94c3"}};
...
void loop(){
  ...
  if (digitalRead(limitSwitch) == 1){
    ...
    if(diff_count == 0 && ((millis()/1000 - time_before) > 10)){
      ...
      detectDefferentialWines(global_CurrentWineTags, global_DifferenceWineTags);
      SERIAL1.println("Result is ...");
      for (int i = 0; i < arrayNumber; i++){
...
//ワインの差分の検出部分
void detectDefferentialWines(char global_CurrentWineTags[tagsNum][idNum], char global_differential_CurrentWineTags[tagsNum][idNum]){
  int k = 1;
  int jj = 0;
  int dz = 0;
  ...
  //////////////////////////
    ...
    for( int di = 0; di<arrayNumber; di++){
      ...
      if(dk == 0){
        strcpy(global_differential_CurrentWineTags[dz], global_CurrentWineTags[di]); 
