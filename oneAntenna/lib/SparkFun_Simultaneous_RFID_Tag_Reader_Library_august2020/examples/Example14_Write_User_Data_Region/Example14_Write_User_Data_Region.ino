/*
  ...
  This is as example 5, but now you can write the data starting from a specific address
...
void setup()
{
  ...
  Serial.println(F("Example14: write user data region"));
...
void loop()
{
  ...
  uint32_t WordStartAdress = 0x3;     // offset 3 x words = start 3 x 2 bytes skip = start at byte  7

  byte responseType = nano.writeUserDataRegion(WordStartAdress, testData, sizeof(testData) - 1); //The -1 shaves off the \0 found at the end of string
